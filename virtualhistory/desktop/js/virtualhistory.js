 ...
 */
 ...
 $('#bt_importEqLogic').on('click', function () {
    jeedom.eqLogic.getSelectModal({}, function (result) {
        $.ajax({// fonction permettant de faire de l'ajax
            ...
            url: "plugins/virtualhistory/core/ajax/virtualhistory.ajax.php", // url du fichier php
            data: {
 ...
 $("#bt_addvirtualhistoryInfo").on('click', function (event) {
    var _cmd = {type: 'info'};
    addCmdToTable(_cmd);
});

 $("#bt_addvirtualhistoryAction").on('click', function (event) {
 ...
 if (init(_cmd.type) == 'info') {
     var disabled = (init(_cmd.configuration.virtualhistoryAction) == '1') ? 'disabled' : '';
     var tr = '<tr class="cmd" data-cmd_id="' + init(_cmd.id) + '" virtualhistoryAction="' + init(_cmd.configuration.virtualhistoryAction) + '">';
     tr += '<td>';
...
if (init(_cmd.type) == 'action') {
    ...
    tr += '<span class="subType" subType="' + init(_cmd.subType) + '"></span>';
    tr += '<input class="cmdAttr" data-l1key="configuration" data-l2key="virtualhistoryAction" value="1" style="display:none;" >';
    tr += '</td>';
    tr += '<td>';
